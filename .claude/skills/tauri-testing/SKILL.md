...
**Verify:** `tauri-driver --version` should print the installed version.
...
**Verify before running tests:** `curl http://127.0.0.1:4444/status` should return a JSON response confirming tauri-driver is listening.

**Verify after building:** The binary should exist at `src-tauri/target/debug/<app-name>`.
...
### Mock Testing
- Call `clearMocks()` in `afterEach` to prevent state leakage
- Use spies to verify IPC calls
- Mock at the right level: IPC for commands, windows for multi-window logic

### WebDriver Testing
- Use debug builds for faster iteration
- Set explicit timeouts; Tauri apps may need time to initialize
- Prefer explicit waits over implicit waits
- Keep tests independent and isolated

### CI Integration
- Use `xvfb-run` on Linux for headless testing
- Match Edge Driver version on Windows
