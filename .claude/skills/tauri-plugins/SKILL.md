...
1. **Scaffold plugin** — `tauri plugin new`
2. **Implement Rust commands** — desktop/mobile split in `commands.rs`, `desktop.rs`, `mobile.rs`
3. **Add permissions** — define TOML permission files and `build.rs`
4. **Write JS bindings** — TypeScript API in `guest-js/index.ts`
5. **Add mobile code** (optional) — Kotlin for Android, Swift for iOS
6. **Register and test** — wire into `tauri::Builder`, verify from frontend
...
```kotlin
trigger("dataReceived", JSObject().apply { put("data", "value") })  // Emit event

override fun onNewIntent(intent: Intent) {  // Lifecycle
...
```swift
trigger("dataReceived", data: ["data": "value"])  // Emit event
...
1. **Rust compilation:** `cargo build` in the plugin crate — confirms commands, types, and error handling compile
2. **Permission generation:** `cargo tauri build` in the host app — confirms `build.rs` generates permission files
3. **Frontend invocation:** Call an exported function from JavaScript and confirm the response:
   ```typescript
   const result = await doSomething('test');
   console.assert(result !== undefined, 'Plugin command returned a result');
   ```
4. **Mobile round-trip** (if applicable): Run on an Android emulator or iOS simulator and invoke a command that hits native code via `run_mobile_plugin`
